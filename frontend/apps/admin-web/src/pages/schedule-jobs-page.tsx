...
export function ScheduleJobsPage({ api }: { api: ReturnType<typeof createApiClient> }) {
  ...
  async function handleServiceAction(action: "start" | "remove", item: SysJobRecord) {
    const actionLabel = action === "start" ? "启动调度服务" : "从调度服务移除";
    if (!window.confirm(`确认对任务「${item.jobName}」执行${actionLabel}吗？`)) {
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "任务服务操作失败";
      ...
      renderAside={() => (
        <SectionCard title="调度说明" description="这里区分任务记录状态与调度服务挂载状态。">
          <ul className="detail-list">
            <li>`状态` 字段决定任务是否允许被启动，`EntryId` 大于 0 代表当前已挂到调度服务。</li>
            <li>`启动调度服务` 和 `从调度服务移除` 不会替代任务编辑，它们只操作运行中的调度注册。</li>
            <li>日志页 `/schedule/log` 仍未迁移，当前先保证任务管理主链路可用。</li>
          </ul>
        </SectionCard>
      )}
      rowActions={(item) => (
        <>
          <button
            ...
            onClick={() => void handleServiceAction("start", item)}
            type="button"
          >
            启动服务
          </button>
          <button
            ...
            onClick={() => void handleServiceAction("remove", item)}
            type="button"
          >
            移除服务
          </button>
